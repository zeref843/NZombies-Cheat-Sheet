...
local color_zct = Color(140, 0, 255, 200)
...
local zct_hud_health = Material("nz_moo/huds/zct/health_hud_back.png", "unlitgeneric smooth")
local zct_hud_healthbar = Material("nz_moo/huds/zct/health_bar.png", "unlitgeneric smooth")
local zct_hud_shieldbar = Material("nz_moo/huds/zct/armour_bar.png", "unlitgeneric smooth")
...
local cod5_icon_special = Material("nz_moo/huds/zct/hud_grenade_launcher_dpad.png", "unlitgeneric smooth")
local cod5_icon_grenade = Material("nz_moo/huds/zct/hud_us_grenade.png", "unlitgeneric smooth")
...
net.Receive("nz_points_notification_zct", function()
...
local function ScoreHud_cod5()
	...
	surface.SetDrawColor(color_zct)
	...
	if nz_shownames:GetBool() then
		...
		if ply:IsSpeaking() then
			...
			surface.SetMaterial(icon)
			surface.SetDrawColor(color_white)
			surface.DrawTexturedRect((w - 8*scale) - tw - 32, h - (260*scale) - offset - 16, 32, 32)
		end
	...
	for k, v in ipairs(plytab) do
		...
		if nz_showhealthmp:GetBool() then
			...
			local armor = v:Armor()
			local hpheight = armor > 0 and 188 or 204

			surface.SetDrawColor(color_white)

			surface.SetMaterial(zct_hud_healthbar)
			surface.DrawTexturedRectUV(w - 216*scale, h - hpheight*scale - offset, 252*0.8*phealthscale*scale, 12*0.6*scale, 0, 0, 1*phealthscale, 1)

			if nz_showhealthmp:GetInt() > 1 then
				draw.SimpleTextOutlined(phealth, ammo2font, w - 234*scale, h - (hpheight - 4)*scale - offset, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black_180)
			end
			...
			if armor > 0 then
				...
				surface.SetMaterial(zct_hud_shieldbar)
				surface.DrawTexturedRectUV(w - 216*scale, h - 204*scale - offset, 252*0.8*armorscale*scale, 12*0.6*scale, 0, 0, 1*armorscale, 1)

				if nz_showhealthmp:GetInt() > 1 then
					draw.SimpleTextOutlined(armor, ammo2font, w - (lowres and 236 or 234)*scale, h - 200*scale - offset, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black_180)
				end
			end
		end
...
local function GunHud_cod5()
	...
	if grenade then
		...
		if IsValid(grenade) and grenade.NZHudIcon then
			icon = grenade.NZHudIcon_cod4 or grenade.NZHudIcon
		end
		if not icon or icon:IsError() then
			icon = zmhud_icon_missing
		end
	...
	if tacnade then
		local icon = cod5_icon_grenade
		if IsValid(tacnade) and tacnade.NZHudIcon then
			icon = tacnade.NZHudIcon_cod4 or tacnade.NZHudIcon_cod5 or tacnade.NZHudIcon
		end
		if not icon or icon:IsError() then
			icon = zmhud_icon_missing
		end
...
local function PowerUpsHud_zct()
	local ply = LocalPlayer()
	if not IsValid(ply) then return end
	...
	local function AddPowerup(powerup, icon, time, anti, noflash) -- Display another powerup on the player's screen
		...
		if !anti and !powerup_data[powerup] then
			powerup_data[powerup] = {[1] = 8*scale, [2] = 1}
		end
		if anti and !antipowerup_data[powerup] then
			antipowerup_data[powerup] = {[1] = 8*scale, [2] = 1}
		end

		local height = 8*scale + (powerupsActive * size)
		local convarstyle = nz_powerupstyle:GetInt()
		if convarstyle > 0 then
			if anti then
				antipowerup_data[powerup][1] = math.Approach(antipowerup_data[powerup][1], height, FrameTime()*160)
				antipowerup_data[powerup][2] = convarstyle > 1 and 0 or math.Approach(antipowerup_data[powerup][2], 0, FrameTime()*3)
			else
				powerup_data[powerup][1] = math.Approach(powerup_data[powerup][1], height, FrameTime()*160)
				powerup_data[powerup][2] = convarstyle > 1 and 0 or math.Approach(powerup_data[powerup][2], 0, FrameTime()*3)
			end
		else
			if anti then
				antipowerup_data[powerup][1] = height
				antipowerup_data[powerup][2] = 0
			else
				powerup_data[powerup][1] = height
		...
		if timeleft > warningthreshold or (timeleft > urgencythreshold and timeleft % (frequency1 * 4) > frequency1) or (timeleft <= urgencythreshold and timeleft % (frequency2*2) > frequency2) then
			local finalheight = anti and antipowerup_data[powerup][1] or powerup_data[powerup][1]
			...
			surface.DrawTexturedRect(scw - (size) - (bonus/2), finalheight + (bonus/2), 64*scale + bonus, 64*scale + bonus)

			if nz_showpoweruptimer:GetBool() then
				draw.SimpleTextOutlined(math.ceil(timeleft), font, scw - (size/2) - (bonus/2), finalheight + 42*scale + (bonus/2), color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, color_black_100)
			end
		end
...
local function PerksHud_cod5()
	...
	local height = centrist and 82 or 150
	local size = 45
	local num = 0
	local row = 0
	local num_b = 0
...
local function ResetRoundPos()
	...
	round_posdata[1] = wscale + 272*scale
	round_posdata[2] = h - 8*scale
end
...
local function GameBeginRound(round)
	...
	hook.Add("HUDPaint", "nz_fuckoffshitt", function()
		if ply.IsInCameraQueue and ply:IsInCameraQueue() then
			...
			if round_posdata[1] ~= (wscale + 272*scale) then
				ResetRoundPos()
			end
			return
		end
...
local function RoundHud_cod5()
	...
	if nz_mapfont:GetBool() then
		font = "nz.main."..GetFontType(nzMapping.Settings.roundfont)
	end
	...
	local color = ColorAlpha(color_white, ouralpha)
	...
	elseif round_posdata[1] ~= (wscale + 272*scale) then
		ResetRoundPos()
	end
	...
	if intro_time < CurTime() and kys_time > CurTime() then
		...
		round_posdata[1] = Lerp(kysratio, wscale + 272*scale, w/2 - 25)
		round_posdata[2] = Lerp(kysratio, h - 8*scale, h/2 + 25*scale)
	end

	draw.SimpleTextOutlined(round_num, font, round_posdata[1], round_posdata[2], color, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 2, color_black_180)
end
...
local function PlayerHealthHUD_cod5()
	...
	surface.SetMaterial(zct_hud_healthbar)
	surface.DrawTexturedRectUV(wr, hr + 32*scale, 256*healthscale*scale, 24*scale, 0, 0, 1*healthscale, 1)
	...
	if armor > 0 then
		...
		surface.SetMaterial(zct_hud_shieldbar)
		surface.DrawTexturedRectUV(wr, hr + 8*scale, 256*armorscale*scale, 24*scale, 0, 0, 1*armorscale, 1)
	end

	surface.SetMaterial(zct_hud_health)
	surface.DrawTexturedRect(wr, hr + 1*scale, 256*scale, 64*scale)

	if nz_showhealth:GetInt() > 1 then
		draw.SimpleTextOutlined(health, smallfont, wr + (fucker and 7 or 8)*scale, hr + (fucker and 36 or 37)*scale, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, color_black_180)
	end

	if armor > 0 then
		if nz_showhealth:GetInt() > 1 then
			draw.SimpleTextOutlined(armor, smallfont, wr + (fucker and 7 or 8)*scale, hr - (32*scale) + (fucker and 36 or 37)*scale, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, color_black_180)
		end
	end
end
...
local function ZedCounterHUD_cod5()
	...
	surface.SetDrawColor(Color(140, 0, 255, 255))
...
local n_lastdifficulty = 0
local f_difficultyratio = 0
local t_difficulties = {
	[1] = {round = 5, icon = Material("nz_moo/huds/zct/zct_difficulty_1.png", "unlitgeneric smooth")},
	[2] = {speed = 36, icon = Material("nz_moo/huds/zct/zct_difficulty_2.png", "unlitgeneric smooth")},
	[3] = {speed = 71, icon = Material("nz_moo/huds/zct/zct_difficulty_3.png", "unlitgeneric smooth")},
	[4] = {speed = 155, icon = Material("nz_moo/huds/zct/zct_difficulty_4.png", "unlitgeneric smooth")},
	[5] = {speed = 0, icon = Material("nz_moo/huds/zct/zct_difficulty_5.png", "unlitgeneric smooth")}
}

local function GenerateSpeedCurve(round)
	local multiplier = nzMapping.Settings.speedmulti or 4
	local speed = nzMapping.Settings.startspeed or 0
	local cap = math.Clamp(nzMapping.Settings.speedcap or 300, 1, 999)

	return math.Clamp(round * multiplier - multiplier, 0, cap) + speed
end

local function DifficultyHUD_zct()
	local hudtype = nzMapping.Settings.hudtype
	if !hudtype or hudtype ~= "Zombie Chicken Taco" then return end

	local num = nzRound:GetNumber() or 0
	if num <= 0 then return end
	local speed = GenerateSpeedCurve(num) or 0
	...
	local size = 46
	local icon = t_difficulties[5] //anything after highest cap in list
	for i, data in ipairs(t_difficulties) do
		if data.round and num < (data.round) then
			if n_lastdifficulty ~= data.round then
				n_lastdifficulty = data.round
				f_difficultyratio = 1
			end
		...
		if data.speed and speed < data.speed then
			if n_lastdifficulty ~= data.speed then
				n_lastdifficulty = data.speed
				f_difficultyratio = 1
			end
	...
	if f_difficultyratio > 0 then
		f_difficultyratio = math.Clamp(f_difficultyratio - RealFrameTime(), 0, 1)
		size = 46 + (8*f_difficultyratio)
	end
	...
	surface.SetFont(font)
	local tw, th = surface.GetTextSize(nzRound:GetNumber())

	surface.SetDrawColor(color_white)
	surface.SetMaterial(icon)
	surface.DrawTexturedRect(wr + tw, hr - (size/2), size*scale, size*scale)
end

-- Hooks
hook.Add("HUDPaint", "nzHUDswapping_zct", function()
	if nzMapping.Settings.hudtype == "Zombie Chicken Taco" then
		...
		hook.Add("HUDPaint", "powerupHUD", PowerUpsHud_zct )
		...
		hook.Add("HUDPaint", "difficultyHUD", DifficultyHUD_zct )
