...
local scoreboard
function nzDisplay:ShowScoreboard(override_x, override_y)
	...
	if nzRound:InState(ROUND_GO) then
		...
		scoreboard:SetPos(override_x or ScrW()/4, override_y or (280*scale) + th)
	else
		scoreboard:SetSize(ScrW()/2, ScrH() - 200)
		scoreboard:SetPos(override_x or ScrW()/4, override_y or 100*scale)
	end
...
function nzDisplay:HideScoreboard()
	if IsValid(scoreboard) then
		scoreboard:Hide()
	end
end

nzDisplay.ScoreboardHold = 0

function GM:ScoreboardShow()
	nzDisplay:ShowScoreboard()
end

function GM:ScoreboardHide()
	if !nzRound:InState(ROUND_GO) or nzDisplay.ScoreboardHold < CurTime() then
		nzDisplay:HideScoreboard()
	end
end
...
hook.Add("OnRoundEnd", "nzu_Scoreboard_ShowOnGameOver", function()
	nzDisplay.ScoreboardHold = CurTime() + nzMapping.Settings.gocamerawait

	timer.Create("gameover_scoreboard_start", nzMapping.Settings.gocamerawait, 1, function()
		nzDisplay:ShowScoreboard()
	end)
	timer.Create("gameover_scoreboard_end", nzRound:GameOverDuration(), 1, function()
		nzDisplay:HideScoreboard()
	end)
end)
