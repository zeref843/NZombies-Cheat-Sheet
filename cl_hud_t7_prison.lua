		...
		if wep.IsTFAWeapon and wep.NZSpecialCategory == "specialist" then
			...
			if wep.NZPickedUpTime and (wep.NZPickedUpTime + 5 > CurTime() or hudbits.fade_special > 0) then
				...
				if wep.NZPickedUpTime + 0.6 > CurTime() then
					surface.SetMaterial(t7_icon_special_swirl)
					surface.SetDrawColor(color_white)
					surface.DrawTexturedRectRotated(w - 135*scale, h - 155*scale, 90*scale, 90*scale, (hudbits.fade_special_acc*900)%360)
				end
			end
		end
...
local function PerksHud_t7_zod()
	...
	if perk_borders > 0 and maxperks > 0 then
		...
		for i=1, ply:GetMaxPerks() do
			local mywidth = centrist and (ScrW()/2) + (num_b*(size + 6)*scale) - (fuck/2)*(size + 6)*scale or (w + num_b*(size + 6)*scale)
			...
			if i > #perks then
				surface.DrawTexturedRect(mywidth, h - height*scale - (64*row_b)*scale, 54*scale, 54*scale)
			end
	...
	if perkflashtime > CurTime() then
		...
		local flash_w = centrist and (ScrW()/2) + ((flashcount-1)*(size + 6)*scale) - (fuck/2)*(size + 6)*scale or (w + ((flashcount-1)*(size + 6))*scale)

		cwimage("rf/round/sparks/" .. (math.ceil(CurTime()*30) % 20) .. ".png", flash_w + 46*scale, h - (height + 54)*scale - 64*flashrow, 168*scale, 252*scale, ColorAlpha(color_white, 255*alpha))

		surface.SetMaterial(t7_hud_score)
		surface.SetDrawColor(ColorAlpha(color_t7_outline, 255*alpha))
		surface.DrawTexturedRect(flash_w - 40*scale, h - (height + 48)*scale - 64*flashrow, 128*scale, 128*scale)
	end
	...
	for i, perk in pairs(perks) do
		...
		local mywidth = centrist and (ScrW()/2 + (num*(size + 6)*scale) - (fuck/2)*(size + 6)*scale) or (w + num*(size + 6)*scale - fuckset*scale)

		surface.SetMaterial(icon)
		surface.SetDrawColor(alpha < 1 and ColorAlpha(perkcolor, 800*alpha) or perkcolor)
		surface.DrawTexturedRect(mywidth, h - (height + fuckset)*scale - (64*row)*scale, 54*pulse*scale, 54*pulse*scale)
		...
		if perk == "vulture" then
			...
			if stinkfade > 0 then
				...
				surface.DrawTexturedRect(mywidth - 24*scale, (h - height*scale - (64*row)*scale) - 24*scale, 102*scale, 102*scale)
